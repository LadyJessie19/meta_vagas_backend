...
export class VacancyService {
    ...
    async findVacancies(page: number, limit: number,
        tech: string, role: string, maxWage: number,
        minWage: number, type: string, local: string,
        description: string) {
        try {
            const vacancies = await this.vacancyRepository.createQueryBuilder("vacancy")
                ...
                .andWhere(new Brackets(qb => {
                    qb.where("vacancy.vacancyDescription ILIKE :role", { role: `%${role?.trim()}%` })
                    .orWhere("vacancy.vacancyRole ILIKE :role", { role: `%${role?.trim()}%` })
